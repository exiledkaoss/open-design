...
/**
 * Write the supplied {providerId: {apiKey, baseUrl}} patch. Omitted
 * providers are preserved, and an explicitly empty provider entry deletes
 * only that provider. Unknown provider IDs are ignored.
 *
 * This keeps a fresh or partial browser profile from replacing the daemon's
 * full on-disk provider map and silently wiping credentials that were set
 * from another browser profile or an earlier session.
 */
