...
/**
 * Apply a provider-level patch. Omitted providers are preserved so a
 * fresh browser profile or partial Settings save cannot wipe existing
 * daemon credentials. Sending an explicit blank entry deletes that one
 * provider.
 */
export async function writeConfig(projectRoot, body) {
  ...
  for (const id of PROVIDER_IDS) {
    ...
    const hasApiKey = Object.prototype.hasOwnProperty.call(entry, 'apiKey');
    const hasBaseUrl = Object.prototype.hasOwnProperty.call(entry, 'baseUrl');
    const apiKey =
      hasApiKey && typeof entry.apiKey === 'string' && entry.apiKey.trim()
        ? entry.apiKey.trim()
        : hasApiKey
          ? ''
          : priorEntry.apiKey || '';
    const baseUrl =
      hasBaseUrl && typeof entry.baseUrl === 'string' && entry.baseUrl.trim()
        ? entry.baseUrl.trim()
        : hasBaseUrl
          ? ''
          : priorEntry.baseUrl || '';
